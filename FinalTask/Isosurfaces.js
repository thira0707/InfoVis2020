function Isosurfaces(volume, isovalue, screen, reflection, r, g, b) {
	...
	for (var i = 0; i < 256; i++) {
		var S = i / 255.0; // [0,1]
		var R = Math.max(Math.cos((S - (r / 2.0)) * Math.PI), 0.0);
		var G = Math.max(Math.cos((S - (g / 2.0)) * Math.PI), 0.0);
		var B = Math.max(Math.cos((S - (b / 2.0)) * Math.PI), 0.0);
		var color = new THREE.Color(R, G, B);
		cmap.push([S, '0x' + color.getHexString()]);
	}
	var material;
	console.log(cmap[isovalue][1]);
	...
	if (reflection == 2) {
		...
		material = new THREE.ShaderMaterial({
			...
			fragmentShader: document.getElementById('phong.frag.lambertian').text,
			uniforms: {
				light_position: { type: 'v3', value: screen.light.position },
				m_color: { type: 'v3', value: materialColor }
			}
	...
	if (reflection == 3) {
		...
		material = new THREE.ShaderMaterial({
			vertexColors: THREE.VertexColors,
			vertexShader: document.getElementById('phong.vert').text,
			fragmentShader: document.getElementById('phong.frag.phong').text,
			uniforms: {
				light_position: { type: 'v3', value: screen.light.position },
				camera_position: { type: "v3", value: screen.camera.position },
				m_color: { type: 'v3', value: materialColor }
